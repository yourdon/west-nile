...
import datetime
import csv
# Lasagne is a Theano-based implementation of NN
from lasagne.layers import InputLayer, DropoutLayer, DenseLayer
from lasagne.updates import nesterov_momentum
from lasagne.objectives import binary_crossentropy
from nolearn.lasagne import NeuralNet
# Theano is a Python library for handling multi-dimensional arrays; tightly integrated with numpy
import theano
...
def load_training(): # from this, we get fields of interest cast into good data types!
    training = []
...
def load_testing(): # same as above; cast into good data types 
    training = []
...
def closest_station(lat, long):
    ...
    # LAT LON below are given and hardcoded
    stations = np.array([[41.995, -87.933],
                         [41.786, -87.752]])
    # get lat lon of current trap
    loc = np.array([lat, long])
    # compute component distances from stations (plural) to trap
    deltas = stations - loc[None, :]
    # square to find the distance
    dist2 = (deltas**2).sum(1)
    # and find out which one is closer
